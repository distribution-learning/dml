...
os.environ['CUDA_VISIBLE_DEVICES'] = conf.get("asy", "client2_CUDA_VISIBLE_DEVICES")
...
def message_cs(direction_flag, thread_name, gradient_w, gradient_b, train_error, loss, sample_size_local, local_epoch, local_duration):
    message_dict = {}
    message_dict['direction_flag'] = direction_flag
    message_dict['thread_name'] = thread_name
    message_dict['gradient_w'] = gradient_w
    message_dict['gradient_b'] = gradient_b
...
def tf_util(gradient_w, gradient_b, w0temp, b0temp, acc, loss):
    loss = float(loss)
    ...
    gradient_w = gradient_w.tolist()
    gradient_w_list.append(gradient_w)

    gradient_b = gradient_b.tolist()
    ...
    return gradient_w, gradient_b, train_error, loss
...
def send_message(thread_name):
    ...
    # 迭代训练
    local_epoch = int(conf.get("asy", "local_epoch"))
    sample_size_local = int(conf.get("asy", "sample_size_local"))
    num_node = int(conf.get("asy", "num_node"))
    client2_sample_number = int(conf.get("asy", "client2_sample_number"))
    for g_epoch in range(global_epoch):
        ...
        for l_epoch in range(local_epoch):
            ...
            # data_train_x, data_train_y = sample_sequence_no_duplication(num_node, client2_sample_number, sample_size_local)
            # data_train_x, data_train_y = sample_sequence_half_duplication(num_node, client2_sample_number, sample_size_local)
            _, gradient, variable, loss = sess.run([train_step, gradients, variables, cross_entropy],
                                                   feed_dict={x: data_train_x, y_: data_train_y})
            # 打乱数据顺序，防止按原次序假性训练输出
            data_train_x, data_train_y = shuffle(data_train_x, data_train_y)

        gradient_w = gradient[0]
        gradient_b = gradient[1]
        ...
        gradient_w, gradient_b, train_error, loss = tf_util(gradient_w, gradient_b, w0temp, b0temp, acc_train, loss)

        print("thread name:", thread_name, "before send global epoch:", g_epoch,
              # "w:", w0temp, "b:", b0temp,
        ...
        message_send = message_cs(1, thread_name, gradient_w, gradient_b, train_error, loss, sample_size_local,
        ...
        # sample_size_local = int(round(message_sc_dict['sample_size_local']))
        # local_epoch = int(round(message_sc_dict['local_epoch']))
        # print("thread name:", thread_name, "after receive global epoch:", g_epoch,
        #       # "w:", message_sc_dict['w'], "b:", message_sc_dict['b'],
        #       "sample_size_local:", sample_size_local, "local_epoch:", local_epoch)
        # 更新tensorflow的w，b
...
if __name__ == '__main__':
    # 设置训练超参数
    # 迭代轮次
    global_epoch = int(conf.get("asy", "client2_global_epoch"))
    # local_epoch = 10
    ...
    # 学习率
    learning_rate = float(conf.get("asy", "learning_rate"))

    # server 的ip和port
    # HOST, PORT = '127.0.0.1', 65433
    HOST, PORT = conf.get("asy", "server_ip"), int(conf.get("asy", "server_port"))
    # 存储每轮全局迭代的中间结果
    ...
    # num_node_syn = int(conf.get("asy", "num_node_syn"))
    # for i in range(0, num_node_syn):
    #     # 创建线程
    #     try:
    #         t = threading.Thread(target=send_message, args=("t2",))
    #         t.start()
    #     except:
