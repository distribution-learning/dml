...
os.environ['CUDA_VISIBLE_DEVICES'] = conf.get("asy", "client4_CUDA_VISIBLE_DEVICES")
...
def send_message(thread_name):
    ...
    client4_sample_number = int(conf.get("asy", "client4_sample_number"))
    for g_epoch in range(global_epoch):
        ...
        for l_epoch in range(local_epoch):
            ...
            # data_train_x, data_train_y = sample_sequence_no_duplication(num_node, client4_sample_number, sample_size_local)
            # data_train_x, data_train_y = sample_sequence_half_duplication(num_node, client4_sample_number, sample_size_local)
...
if __name__ == '__main__':
    # 设置训练超参数
    # 迭代轮次
    global_epoch = int(conf.get("asy", "client4_global_epoch"))
    # local_epoch = 10
    ...
    #     # 创建线程
    #     try:
    #         t = threading.Thread(target=send_message, args=("t4",))
    #         t.start()
    #     except:
