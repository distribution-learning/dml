...
os.environ['CUDA_VISIBLE_DEVICES'] = conf.get("hybrid", "client2_CUDA_VISIBLE_DEVICES")
...
def send_message(thread_name):
    ...
    # 迭代训练
    local_epoch = int(conf.get("hybrid", "local_epoch"))
    sample_size_local = int(conf.get("hybrid", "sample_size_local"))
    num_node = int(conf.get("hybrid", "num_node"))
    client2_sample_number = int(conf.get("hybrid", "client2_sample_number"))
    for g_epoch in range(global_epoch):
        ...
        for l_epoch in range(local_epoch):
            ...
            # sess.run(train_step, feed_dict={x: data_train_x, y_: data_train_y})
        ...
        # 更新tensorflow的w，b
        # w = tf.convert_to_tensor(message_sc_dict['w'])
        # b = tf.convert_to_tensor(message_sc_dict['b'])
        w_recv = (message_sc_dict['w'])[1]
        b_recv = (message_sc_dict['b'])[1]
        sess.run(tf.assign(w, w_recv))
        sess.run(tf.assign(b, b_recv))
...
if __name__ == '__main__':
    # 设置训练超参数
    # 迭代轮次
    global_epoch = int(conf.get("hybrid", "client2_global_epoch"))
    # local_epoch = 10
    ...
    HOST, PORT = conf.get("hybrid", "server_ip_syn1"), int(conf.get("hybrid", "server_port_syn1"))
    # 存储每轮全局迭代的中间结果
    ...
    t3 = threading.Thread(target=send_message, args=('t1', ))
    t3.start()

    # t4 = threading.Thread(target=send_message, args=('t2', ))
    # t4.start()
